...
def upgrade() -> None:
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("section_number"),
    )

    op.create_table(
        "mtg_rule_subsection",
        sa.Column("section_number", sa.INTEGER(), nullable=False),
        sa.Column("subsection_number", sa.INTEGER(), nullable=False),
        sa.Column("subsection_title", sa.TEXT(), nullable=False),
        sa.ForeignKeyConstraint(["section_number"], ["mtg_rule_section.section_number"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("section_number", "subsection_number"),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["section_number"], ["mtg_rule_section.section_number"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(
            ["section_number", "subsection_number"],
            ["mtg_rule_subsection.section_number", "mtg_rule_subsection.subsection_number"],
            ondelete="SET NULL",
        ),
    ...
    op.create_index(
        ...
        "mtg_rule_section",
        [sa.text("to_tsvector('english', section_title)")],
    ...
    op.create_index(
        ...
        "mtg_rule_subsection",
        [sa.text("to_tsvector('english', subsection_title)")],
        postgresql_using="gin",
    )

    op.create_index("idx_section", "mtg_rule", ["section_number", "subsection_number"])
    op.create_index(
